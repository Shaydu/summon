# Summon API Documentation (v3.3)

This document describes the Summon API (revision v3.3). It consolidates all current server contracts, client implementation details, validation rules, batch sync semantics, and NFC operations.

## Terminology
- **Actor**: the player/account who initiates the action (causes an item to be given or an entity to be spawned). Maps to `summoning_player` in the immediate `/summon` endpoint and to `player_id` in v3.3 sync payloads.
- **Target**: the player who receives the action (the item is given to, or the entity is spawned nearby). Maps to `summoned_player` on the immediate `/summon` endpoint. The v3.3 sync payload captures the actor; server-side logic may map to target context when executing.
...
BatchErrorResponse schema (v3.3):
...
- `metadata`: required object. If present, `custom_name` (max 32 chars) and `level` (int, 1..100) are validated if provided.

---

## Supported Object Types

The following object types are currently supported in the iOS client and can be used for `summon_type`, `minecraft_id`, and `summoned_object_type` fields:

### Hostile Mobs (Characters)
| Entity Name | Minecraft ID | Default Hand Items | Notes |
|------------|--------------|-------------------|-------|
| Zombie | `zombie` | `{id:diamond_sword,Count:1}` | Spawns with diamond sword |
| Skeleton | `skeleton` | `{id:bow,Count:1}` | Spawns with bow |
| Piglin | `piglin` | `{id:golden_sword,Count:1}` | Spawns with golden sword |
| Vex | `vex` | None | Flying hostile mob |
| Pillager | `pillager` | None | Spawns naturally armed |
| Vindicator | `vindicator` | None | Spawns naturally armed |
| Wither Skeleton | `wither_skeleton` | None | Nether mob |
| Stray | `stray` | None | Skeleton variant |
| Hoglin | `hoglin` | None | Nether mob |
| Ravager | `ravager` | None | Large hostile mob |
| Ender Dragon | `ender_dragon` | None | Boss mob |

### Items
Any Minecraft item ID can be used with the `give_` prefix for item commands. Common examples:
- `give_diamond_sword` - Gives a diamond sword to the player
- `give_bow` - Gives a bow to the player
- `give_golden_sword` - Gives a golden sword to the player

### Custom Types
The API accepts any non-empty string up to 64 characters for `summon_type`. Server-side implementations may validate against known Minecraft entity and item types. The client no longer restricts to a whitelist, allowing for:
- New Minecraft entities added in future updates
- Custom modded entities
- Server-specific custom types

**Note**: When using the immediate `/summon` endpoint, the `entity_summoned` field is automatically excluded for commands with a `give_` prefix (items), and included for character/NPC summons.

---

## Client-side guarantees and behavior (v3.3)
...
### Caching Strategy
- Player list (/players endpoint) cached for 5 minutes
...
Retrieves a list of active players from the server. Used by the client to populate player selection dropdowns and validate player names.
...
Notes:
- Response uses `players` key (standardized from v3.2's `users` key)
- Client caches response for 5 minutes to reduce redundant requests
- Returns empty array `[]` if no players are active
  ...
  "summons": [
    {
      ...
      "summon_time": "2025-12-22T15:00:00Z",
      "location": { "x": 100.5, "y": 64.0, "z": -200.0 },
      "metadata": { "custom_name": "ZombieBoss", "level": 10 }
    },
    {
      "token_id": "def456",
      "player_id": "ActorTwo",
      ...
      "metadata": { "custom_name": "Skelly", "level": 5 }
    }
  ]
}
```
