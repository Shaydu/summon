...
## Terminology
- **Actor**: the player/account who initiates the action (causes an item to be given or an entity to be spawned). Maps to `summoning_user` in the immediate `/summon` endpoint and to `player_id` in v3.3 sync payloads.
- **Target**: the player who receives the action (the item is given to, or the entity is spawned nearby). Maps to `summoned_user` on the immediate `/summon` endpoint. The v3.3 sync payload captures the actor; server-side logic may map to target context when executing.
